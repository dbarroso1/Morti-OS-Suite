...
conversations = open(("corpus/movie_lines.txt"), encoding = 'utf-8', errors = 'ignore' ).read().split('\n')
...
while(True):
    ...
    question = question + \
        [prep.questionswords2int['<PAD>']] * (25 - len(question))
    fake_batch = np.zeros((prep.batch_size, 25))
    fake_batch[0] = question
    predicted_answer = session.run(prep.test_predictions, {
                                    prep.inputs: fake_batch, prep.keep_prob: 0.5})[0]
